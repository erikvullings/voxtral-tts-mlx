...
class RealKugelAudioEngine:
    ...
    # The MLX KugelAudio port does not implement voice conditioning: the `voice`
    # parameter in Model.generate() is accepted but ignored (the upstream PyTorch
    # model injects pre-encoded acoustic_mean embeddings into the prompt, which the
    # MLX port omits entirely). As a stopgap, we seed MLX's RNG per voice name so
    # output is at least deterministic and consistent within a voice selection.
    _VOICE_SEEDS: dict[str, int] = {"default": 42, "warm": 7, "clear": 13}
    ...
    def synthesize(
        ...
        **kwargs: Any,
        ...
        # Seed MLX RNG per voice so output is consistent even though the MLX
        # model currently ignores the voice parameter (voice conditioning is
        # not implemented in the MLX port).
        import mlx.core as mx

        mx.random.seed(self._VOICE_SEEDS.get(voice, 42))
