...
```bash
python scripts/encode_voices_kugelaudio.py --input my_voices --output voices
```
...
Use the `synthesize_with_voice.py` script to generate speech:

```bash
# Basic usage (default: "anouk" voice, sample text)
python scripts/synthesize_with_voice.py
...
mkdir my_voices
cp /path/to/your/voice_samples/*.wav ./my_voices/
...
# 5. Synthesize speech with a voice
python scripts/synthesize_with_voice.py \
  --voice anouk \
...
### Batch Processing
Encode and synthesize multiple voices/texts:

```bash
for voice in voices/*.pt; do
  name=$(basename $voice .pt)
  python scripts/synthesize_with_voice.py --voice $name --output output_$name.mp3
done
```
